...
export function BoardReportExportMenu({ period }: { period: ReportPeriod }) {
        ...
        {open && (
          <>
            ...
            <motion.div
              ...
              className="absolute right-0 z-50 mt-1 w-48 overflow-hidden rounded-2xl border border-border bg-card p-1.5 shadow-card"
            >
              {OPTIONS.map((opt) => (
                <a
                  key={opt.format}
                  href={`/api/export/board-report/${period}?format=${opt.format}`}
                  onClick={() => setOpen(false)}
