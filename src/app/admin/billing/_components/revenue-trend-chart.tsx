 ...
 * and src/app/dashboard/analytics/_components/line-trend.tsx. Colocated
 * here rather than cross-imported across route-private _components
 * folders (that repo's own convention), rather than pulling in a charting
 * library.
 */
export function RevenueTrendChart({
  ...
  if (points.length === 0) {
    return <p className="py-8 text-center text-sm text-muted-foreground">Not enough payment history yet.</p>;
  }
  ...
  const ariaLabel = `Revenue trend line chart from ${points[0].label} to ${points[points.length - 1].label}, ranging from ${formatValue(dataMin)} to ${formatValue(dataMax)}`;
