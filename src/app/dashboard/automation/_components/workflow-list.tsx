          ...
          {workflows.map((workflow) => (
            <tr key={workflow.id} className="border-b border-border/60 align-top">
              <td className="py-3 pr-4">
                <Link href={`/dashboard/automation/workflows/${workflow.id}`} className="font-medium text-foreground hover:underline">
                  {workflow.name}
                </Link>
              ...
              <td className="py-3 pr-4 text-muted-foreground">
                <Link href={`/dashboard/automation/workflows/${workflow.id}/runs`} className="hover:text-foreground hover:underline">
                  {workflow.runCount}
                </Link>
              </td>
              <td className="py-3 pr-4 text-muted-foreground">
                  ...
                  <Button
                    ...
                    onClick={() =>
                      startTransition(async () => {
                        ...
                        if (result.ok && result.workflowId) router.push(`/dashboard/automation/workflows/${result.workflowId}`);
                        else router.refresh();
                      })
                    }
                  >
