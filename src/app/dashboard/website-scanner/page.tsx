...
import { listKeywordResearch, type SeoKeyword } from "@/lib/seo/agent";
...
import { KeywordResearchPanel } from "./_components/keyword-research-panel";
...
export default async function WebsiteScannerPage() {
  ...
  const [stats, scans, keywordResearch] = await Promise.all([
    ...
    listKeywordResearch(membership.organizationId),
  ]);

  const keywordResearchRows = keywordResearch.map((r) => ({
    id: r.id,
    topic: r.topic,
    keywords: r.keywords as unknown as SeoKeyword[],
    ...
    <main className="py-8">
      <Container className="flex flex-col gap-6">
        ...
        <Tabs defaultValue="scans">
          <TabsList>
            <TabsTrigger value="scans">Website Scans</TabsTrigger>
            <TabsTrigger value="keywords">SEO Agent — Keyword Research</TabsTrigger>
          </TabsList>

          <TabsContent value="scans" className="flex flex-col gap-6">
            <ScanForm />
          ...
          <TabsContent value="keywords">
            <KeywordResearchPanel initialResearch={keywordResearchRows} />
          </TabsContent>
        </Tabs>
      </Container>
    </main>
