/**
 * Minimal, dependency-free SVG line chart — single hue (primary), matches
 * the app's existing bar-chart convention (Weekly Performance on the
 * dashboard home) rather than pulling in a charting library. Each point
 * carries a native <title> tooltip since there's no dedicated hover layer
 * here — honest minimal interactivity, not a fake static image.
 */
export function LineTrend({
  ...
  const ariaLabel = `Line chart from ${points[0].label} to ${points[points.length - 1].label}, ranging from ${formatValue(dataMin)} to ${formatValue(dataMax)}`;
  ...
  return (
    <div className="flex flex-col gap-1">
      <svg viewBox={`0 0 ${width} ${height}`} className="h-auto w-full" preserveAspectRatio="none" role="img" aria-label={ariaLabel}>
        ...
        {coords.map((c) => (
          <circle key={c.label} cx={c.x} cy={c.y} r={3} fill="var(--color-primary)">
            <title>
