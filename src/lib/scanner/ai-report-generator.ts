 ...
 * real AI call (through the fallback chain, src/lib/ai/fallback.ts)
 * reasoning strictly over the real findings already persisted for this scan
 * (see seo/performance/security/ux-analyzer.ts + opportunity-score.ts).
 * Unlike Lead Finder's runWebSearchDiscovery, this does NOT use the
 * web_search tool — the real page has already been fetched, so the model
 * reasons over ground truth we already hold, exactly like
 * recommendations.ts's generateStructured pattern. Throws
 * AINotConnectedError / AllAIProvidersFailedError on failure — callers
...
export async function generateExecutiveReport(scanId: string): Promise<ExecutiveReport & { recommendations: ScanRecommendation[] }> {
  ...
  try {
    const result = await generateStructured({
      ...
      maxTokens: 4096,
      effort: "medium",
      schema: ExecutiveReportResponseSchema,
    });
