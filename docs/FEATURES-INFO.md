...
- **Google / Microsoft / GitHub / LinkedIn se login (OAuth)** — 🔑 NEEDS CONFIG — `GOOGLE_CLIENT_ID`, `MICROSOFT_ENTRA_ID_CLIENT_ID`, `GITHUB_CLIENT_ID`, `LINKEDIN_CLIENT_ID` (+ secrets) set karne honge. Button khud-ba-khud sirf tabhi dikhta hai jab uska provider actually configured ho (`getEnabledOAuthProviders()`). **2026-07-29 tak ek real bug tha**: `docker-compose.yml` ke `environment:` block me in char OAuth providers ka koi bhi env var pass hi nahi hota tha — `.env` me set karne ke bawajood container tak pahunchte nahi the (same class ka bug jo pehle `EMAIL_SERVER`/`RESEND_API_KEY` me mila tha). Ab fix ho chuka, aur isi din LinkedIn OAuth (`next-auth`'s built-in LinkedIn OIDC provider) naya add kiya gaya — pehle sirf backend-ready teen providers the, LinkedIn ka to adapter hi nahi tha.
